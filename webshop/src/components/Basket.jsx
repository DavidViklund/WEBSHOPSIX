...
const Basket = () => {
  const { cartItems, removeFromCart, updateQuantity, removeAllFromCart, setOrderSummaryItems } = useCheckout();
  const getTotalPrice = () => {
  ...
  return (
    <div className="w-full flex justify-center py-8">
      <div className="w-full max-w-[1440px] px-4 md:px-8 lg:px-40">
        <div className="flex">
          <BackButton to={'/'} icon={<span className="material-symbols-outlined">arrow_back</span>} text={'Back to start'} />
        </div>
        {cartItems.length === 0 ? (
          ...
          <div className="flex flex-col justify-center gap-8">
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-wrap justify-center gap-4">
                {cartItems.map((item) => (
                  <div key={item.id} className="w-[360px] p-4 flex flex-col bg-greyscale-surface-subtle border border-greyscale-border-default rounded-xl">
                    <div className="flex items-center justify-center bg-white rounded-lg border border-greyscale-border-default">
                      <img src={item.image} alt={item.title} className="h-[250px] object-contain" />
                    </div>
                    <div className="flex flex-col justify-between flex-grow mt-4">
                      <div className="flex justify-between items-start space-x-2 mb-4">
                        <h5 className="font-bold">{item.title.split(' ').slice(0, 4).join(' ')}</h5>
